    ...
    if page == "Check a Package":
        ...
        if st.button("Analyze") and pkg_name:
            ...
            with col3:
                ...
                st.write("Age since first release (days):", int(row["t_age_first_release_days"].values[0]))
                st.write("Age since last release (days):", int(row["t_age_last_release_days"].values[0]))

            # --- Key signals sections ---
            st.markdown("### Key signals")

            with st.expander("Activity & Downloads"):
                st.write(row[["n_downloads_7d","n_downloads_30d","n_versions",
                              "has_single_release","t_age_first_release_days",
                              "t_age_last_release_days","n_distributions"]])

            with st.expander("Repository & Ecosystem"):
                st.write(row[["has_repo_url","cat_repo_host",
                              "n_dependents_est","min_dep_lev_to_brand","n_requires"]])

            with st.expander("Maintainers"):
                st.write(row[["n_maintainers","n_low_download_pkgs_by_maintainers",
                              "n_pkgs_by_maintainers_30d"]])

            with st.expander("Description & Metadata"):
                st.write(row[["n_desc_len","n_desc_lines","n_summary_len","n_classifiers",
                              "has_homepage","dist_embed_to_legit_desc"]])
        ...
        mask = (df["risk_score"] >= min_score) & (df["n_downloads_30d"] >= min_downloads)
        df_filtered = df.loc[mask, [
            "risk_score","risk_label","n_downloads_7d","n_downloads_30d",
            "n_versions","has_single_release","cat_repo_host","n_dependents_est","n_maintainers"
